...
func main() {
	n := 123
	p := &n
	fmt.Println(reflect.ValueOf(n)) // false false
	...
	valueof_pointer_elem := valueof_pointer.Elem()                   // 取得vp的底层指针值引用的值的代表值
	...
	valueof_pointer_elem.Set(reflect.ValueOf(789))                             // <=> vn.SetInt(789)
	fmt.Println(n)                                                             // 789
	...
	fmt.Println(struct_pointer_elem)
